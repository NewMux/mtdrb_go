 ...
 *
 * The `.web.ts` suffix, not a runtime check, is what keeps sql.js out of the
 * native bundle: sql.js's Node build pulls in `fs`, which Metro cannot
 * resolve for iOS/Android, and it fails to resolve at bundle time regardless
 * of whether the code importing it ever runs. `./sqljs.ts` is the stub Metro
 * picks for every other platform.
 */
