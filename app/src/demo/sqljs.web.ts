 ...
 *
 * The `.web.ts` suffix covers the case the flag cannot: a demo dev server
 * that Expo Go connects to. sql.js's Node build pulls in `fs`, which Metro
 * cannot resolve for iOS/Android whether or not the code ever runs, so
 * `./sqljs.ts` is the stub Metro picks for every other platform.
 */
